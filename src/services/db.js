...
export const dbService = {
    ...
    migrateData() {
        // Check if materials library is empty and seed if needed
        const materialCount = this.queryOne('SELECT COUNT(*) as count FROM materials_library');
        if (!materialCount || materialCount.count === 0) {
            console.log('Migrating: Seeding materials library...');
            this.seedMaterialsLibrary();
        }

        // Check if categories need updating (add new construction categories)
        const categoryCount = this.queryOne('SELECT COUNT(*) as count FROM categories WHERE name = ?', ['Electrical']);
        if (!categoryCount || categoryCount.count === 0) {
            console.log('Migrating: Updating categories...');
            this.seedCategories();
        }
